    ...
    <html>
      ...
      <body>
        ...
        <form action="/save" method="POST">
          ...
          <button><a style='color:#FFFFFF' href=${host}:${port}/find>CERCA</a></button>&nbsp;
          <button><a style='color:#FFFFFF' href=${host}:${port}/list>VISULIZZA TUTTI</a></button>
        </form>
      </body>
    </html>
  `);
...
app.get("/list", async(req, res) => {
    ...
    <html>
      ...
      <h3>Elenco record inseriti</h3>
       ${myresult}
       <button><a style='color:#FFFFFF' href=${host}:${port}>HOME</a></button>&nbsp;
       <button><a style='color:#FFFFFF' href=${host}:${port}/find>CERCA</a></button>
...
app.post('/mysearch', async(req, res) => {
    ...
    <html>
      ...
      <h3>RISULTATO DELLA RICERCA per ${enteredText}</h3>
       ${myresult}
      <button><a style='color:#FFFFFF' href=${host}:${port}/>HOME</a></button>&nbsp;
      <button><a style='color:#FFFFFF' href=${host}:${port}/find>FAI UN'ALTRA RICERCA</a></button>
      <button><a style='color:#FFFFFF' href=${host}:${port}/list>VISULIZZA TUTTI</a></button>
